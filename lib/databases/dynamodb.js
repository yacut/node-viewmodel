...
_.extend(DynamoDB.prototype, {
  ...
  get: function (id, callback) {
    ...
    this.client.query(params, function (err, data) {
      debug('get', err);
      debug('get', JSON.stringify(data, null, 2));
      if (err) {
        return callback(err);
      }
  ...
  find: function(query, queryOptions, callback) {
    ...
    this.client.scan(params, function(err, vms) {
      ...
      debug('find vms', JSON.stringify(vms, null, 2));
  ...
  checkConnection: function() {
    ...
    waitForTableExists(this.client, this.options.tableName, function(err, data) {
      if (err) {
  ...
  clearAll: function (callback) {
    async.each(collections, function(tableName, done){
      clearTable(this.client, tableName, done);
    }, callback);
  },
...
var clearTable = function(client, tableName, callback) {
  async.waterfall([
    function(done){
      client.scan({ TableName: tableName }, done);
    },
    function(data, done){
